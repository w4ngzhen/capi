 ...
 * @param painter
 */
void ExploreLayer::paintAssistantPanelAt(int panelX, int panelY, QPainter &painter)
{
    painter.save();
    ...
    int top = mY - PADDING;
    // +1是因为要把鼠标所在位置的1个像素加上
    QRect partialRect = QRect(left, top, PADDING * 2 + 1, PADDING * 2 + 1);
    ...
    const QRect ASSISTANT_PANEL_RECT = QRect(ASSISTANT_PANEL_X, ASSISTANT_PANEL_Y,
                                       ASSISTANT_PANEL_W, ASSISTANT_PANEL_H);
    ...
    painter.drawImage(QRect(SCALED_IMG_X,
                            SCALED_IMG_Y,
                            SCALED_IMG_W,
                            SCALED_IMG_H),
                      partialImage);
    ...
    // 计算放大倍数
    qreal scale = SCALED_IMG_W / partialRect.width();
    // 设置反色模式
    painter.setCompositionMode(QPainter::RasterOp_SourceAndNotDestination);
    painter.setPen(QPen(Qt::white, scale));
    // 设置十字路径并绘制
    QPainterPath path;
